...
const findIndexOfOptimumCardMatchingSuiteOfHand = (hand, currentRound, trumpSuite) => {
  const trumpCards = _.filter(hand, (h) => h.suite === trumpSuite);
  if (trumpCards.length > 1) {
    const maxTrumpCard = _.maxBy(trumpCards, 'value');
...
// TODO:
/*
  If you have a card of suit of the hand
    Put max if you can win based on cards on table so far
    Put min if you can’t win
If you have trump and can win based on cards on table so far 
    Put the max face value trump 
...
const playCard = (hand, cardsLeft, currentRound, trumpSuite) => {
  if (currentRound.length > 1) {
    ...
    indexOfCardToPlay = findIndexOfOptimumCardMatchingSuiteOfHand(hand, currentRound, trumpSuite);
