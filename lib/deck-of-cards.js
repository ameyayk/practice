...
const findOptimumCardMatchingSuiteOfHand = (hand, currentRound) =>
  // TODO:
  /*
...
const playCard = (hand, cardsLeft, currentRound) => {
  if (currentRound.length > 1) {
    let indexOfCardToPlay = -1;

    indexOfCardToPlay = findOptimumCardMatchingSuiteOfHand(hand, currentRound);
...
module.exports.playGame = (numberOfPlayers) => {
  ...
  for (let rnd = 0; rnd < numberOfRounds; rnd += 1) {
    ...
    while (tempCurrentPlayingOrder.length > 0) {
      ...
      currentRound.push({
        card: playCard(dealtHands[currentPlayerIndex], numberOfRounds - rnd, currentRound),
